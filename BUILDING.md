...
The C++ suite covers folder browsing, sequential queues, input layout and validation, file drops, mail setup, history, and Explorer COM selection. The selection test registers a temporary test-only COM class in its own process and verifies delivery of 20 Unicode paths without changing the user's registry.
